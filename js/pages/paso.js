...
const coloresAgrupacionesPoliticas = {
  ...
  5: { oscuro: graficaGris, claro: graficaGrisClaro },
  6: { oscuro: graficaVerde, claro: graficaVerdeClaro },
...
async function cardAgrupaciones() {
    try{
        datosFiltrados.valoresTotalizadosPositivos.forEach((agrupacion) => {
            ...
            // PONGO EL NOMBRE DE LA AGRUPACION:

            let listaAgrupaciones = document.getElementById('lista-agrupaciones');
            let nombreAgrup = document.createElement('h4');
            nombreAgrup.innerHTML = agrupacion.nombreAgrupacion + '<hr>';
            listaAgrupaciones.appendChild(nombreAgrup);

            // POR CADA LISTA, PONGO EL NOMBRE Y CANT. DE VOTOS CON NUMERO Y BARRA.

            agrupacion.listas.forEach((lista) => {
                let nombreSubagrup = document.createElement('h5');
                nombreSubagrup.innerText = lista.nombre;
                listaAgrupaciones.appendChild(nombreSubagrup);
                let votosSubagrup = document.createElement('p');
                let calculoVotos = (lista.votos / agrupacion.votos) * 100;
                votosSubagrup.innerHTML = calculoVotos.toFixed(2) + "% <br>" + lista.votos + " votos";
                listaAgrupaciones.appendChild(votosSubagrup);

                // CODIGO PARA LA BARRA DE PORCENTAJE:
                ...
                progressBar.style.width = calculoVotos.toFixed(2) + '%';
                ...
                progressBarText.innerText = calculoVotos.toFixed(2) + "%";
                progressBarText.style.overflow = 'hidden';
                progressBar.appendChild(progressBarText);
            })

            // ESPACIO EN BLANCO PARA SEPARAR LA PRÓXIMA AGRUPACIÓN DE LA ANTERIOR

            espacioBlanco = document.createElement('br');
            listaAgrupaciones.appendChild(espacioBlanco);
        })
    }

    catch{
        console.log('Error al utilizar cardAgrupaciones()');
    }
}
